...
import pandas as pd
import numpy as np
import config  # V69: TRAIL_EMA_SPAN (constantes; sin import circular)
...
def calcular_indicadores(df):
    ...
    # --- V69 (2026-07-11): columnas para el trailing por indicador. Causales,
    # vectorizadas, inofensivas si los flags están OFF (mismo patrón que arriba).
    # T2: EMA fija para el trailing (default span 20 vía config).
    _ema_span = getattr(config, 'TRAIL_EMA_SPAN', 20)
    df[f'EMA_{_ema_span}'] = df['close'].ewm(span=_ema_span, adjust=False).mean()
    # T3 "caimanada": low/high de la vela DIARIA COMPLETADA anterior (D-1),
    # conocido en cualquier momento del día D → causal, sin lookahead. El día
    # actual (aún incompleto) NUNCA se usa: shift(1) sobre el agregado diario.
    try:
        _d = df.set_index('time').resample('1D').agg({'low': 'min', 'high': 'max'})
        _prev_low = _d['low'].shift(1)
        _prev_high = _d['high'].shift(1)
        _key = df['time'].dt.normalize()
        df['Prev_Daily_Low'] = _key.map(_prev_low).values
        df['Prev_Daily_High'] = _key.map(_prev_high).values
    except Exception:
        df['Prev_Daily_Low'] = float('nan')
        df['Prev_Daily_High'] = float('nan')
    return df
