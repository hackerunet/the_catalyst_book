...
def _salidas_vela_mc(t, row, ts, tendencia_ahora=None):
    ...
    if getattr(config, 'EXIT_MODE', 'escalera') == 'tendencia':
        ...
        # V69 (2026-07-11): mismo trailing por indicador que BacktestV25._salidas_vela,
        # para que el null comparta EXACTAMENTE la misma salida que la señal real.
        nivel_trail = None
        ...
        if nivel_trail is not None and nivel_trail == nivel_trail:
            if t['type'] == 'LONG':
...
def main():
    ...
    ap.add_argument('--trail-vwap', action='store_true',
                    help='V69 T1: trailing por VWAP anclado desde la entrada '
                         '(activa TRAIL_VWAP_TENDENCIA)')
    ap.add_argument('--trail-ema', action='store_true',
                    help='V69 T2: trailing por EMA fija (span TRAIL_EMA_SPAN=20, '
                         'idéntica en todos los símbolos; activa TRAIL_EMA_TENDENCIA)')
    ap.add_argument('--trail-swing-htf', action='store_true',
                    help='V69 T3 "caimanada": trailing al mínimo/máximo de la vela '
                         'DIARIA completada anterior (activa TRAIL_SWING_HTF_TENDENCIA)')
    ...
    if args.trail_vwap:
        config.TRAIL_VWAP_TENDENCIA = True
    if args.trail_ema:
        config.TRAIL_EMA_TENDENCIA = True
    if args.trail_swing_htf:
        config.TRAIL_SWING_HTF_TENDENCIA = True
    if args.symbols:
