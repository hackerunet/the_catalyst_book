...
def main():
    print("Iniciando Motor M1 - Cross-Sectional Momentum (V44)...")
    ...
    lookback = 14 # 14 días de retorno para calcular el Momentum
    top_k = 2     # Comprar los 2 más fuertes
    bottom_k = 2  # Vender en corto los 2 más débiles
    ...
    for t in range(lookback, len(prices)-1):
        ...
        # Long top k
        longs = ranks[ranks <= top_k].index
        for sym in longs:
            target_weights[sym] = 0.5 / top_k
            
        # Short bottom k
        shorts = ranks[ranks >= (len(symbols) - bottom_k + 1)].index
        for sym in shorts:
    ...
    print("\n--- Resultados V44: Momentum Cross-Sectional ---")
