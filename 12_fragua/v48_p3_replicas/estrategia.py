...
# Estado global para implementar "Cerrar y Replicar" (P3) de forma stateless
# respecto al Motor Honesto inalterado. Solo actúa como un puente de 1 vela.
_replica_queue = {}
...
def evaluar_entrada(df, rs_basket=None, rs_symbol=None, reentrada_armada=False):
    """
    PURA — no muta estado, ni tiene memoria. Retorna un diccionario con
    el trade props propuesto, o None si no hay entrada en esta vela.
    """
    if len(df) < 50:
        return None

    # P3: Lógica de replicación (Stateless bridging).
    # Si este símbolo fue cerrado por ganancia de réplica en la misma vela,
    # re-ingresar inmediatamente en la misma dirección sin chequear patrones.
    if rs_symbol in _replica_queue:
        req = _replica_queue.pop(rs_symbol)
        dir_replica = req['dir']
        count = req['count']
        ...
        tp = precio + dist_tp if dir_replica == 'LONG' else precio - dist_tp
        sl = precio - dist_sl if dir_replica == 'LONG' else precio + dist_sl
        ...
        return {
            ...
            'sl': sl,
            'dist_sl': dist_sl,
            'pattern': f'RÉPLICA (P3) - {count}',
            'patrones_detectados': [],
            'prob_reversion': prob_reversion(df, dir_replica),
        }
...
def evaluar_salida(t, precio):
    ...
    if getattr(config, 'EXIT_MODE', 'escalera') == 'tendencia':
        sym = t.get('symbol')
        if prog >= getattr(config, 'REPLICA_ARM_PROGRESS', 150.0) and sym:
            count = 0
            if str(t.get('pattern', '')).startswith('RÉPLICA (P3)'):
                try:
                    count = int(t['pattern'].split('-')[-1].strip())
                except ValueError:
                    pass
            if count < getattr(config, 'MAX_REPLICAS', 1):
                _replica_queue[sym] = {'dir': t['type'], 'count': count + 1}
                res['cerrar'] = f'REPLICA P3 (Meta alcanzada, {count+1}/{getattr(config, "MAX_REPLICAS", 1)})'
        return res
