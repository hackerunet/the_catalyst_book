...
# ---------------------------------------------------------------------------
# Detector de reverso CALIBRADO (calibrar_reverso.py, 2026-06-24)
# Regresión logística sobre 25.285 estados reales (3 años), split temporal:
# entrenado en el 70% más viejo, validado OOS en el 30% reciente → corr +0.178
# (el heurístico viejo: +0.023), calibración OOS MONOTÓNICA. El feature
# dominante es la sobre-extensión vs EMA200. Devuelve la probabilidad REAL de
# reverso (ya es la tasa observada por quintil), no un puntaje arbitrario.
# ---------------------------------------------------------------------------
_DET_MU = {'overext': 5.432395, 'rsi_exh': 3.300264, 'vol_ratio': 1.101732,
           'body_atr': 0.487968, 'adx': 28.794926}
_DET_SD = {'overext': 3.841682, 'rsi_exh': 4.885627, 'vol_ratio': 0.756744,
           'body_atr': 0.42491, 'adx': 11.75033}
_DET_W = {'intercept': -0.886004, 'overext': 0.396061, 'rsi_exh': -0.012813,
          'vol_ratio': 0.098562, 'body_atr': 0.08504, 'adx': -0.027215}
_DET_FEATS = ['overext', 'rsi_exh', 'vol_ratio', 'body_atr', 'adx']


def _features_reverso(df, tipo_posicion):
    """Mismos features (causales, direccion-ajustados) que calibrar_reverso.py."""
    r = df.iloc[-1]
    close, e200 = r['close'], r['EMA_200']
    rsi = r['RSI']
    atr = r['ATR'] if r['ATR'] and not pd.isna(r['ATR']) else None
    vma = r['Volume_MA']
    if pd.isna(e200) or not e200:
        return None
    if tipo_posicion == 'SHORT':
        overext = -(close - e200) / e200 * 100
        rsi_exh = max(0.0, 40.0 - rsi)
    else:
        overext = (close - e200) / e200 * 100
    ...
    return {
        'overext': overext,
        'rsi_exh': rsi_exh,
        'vol_ratio': float(r['volume'] / vma) if (vma and not pd.isna(vma) and vma > 0) else _DET_MU['vol_ratio'],
        'body_atr': float(r['Body'] / atr) if atr and atr > 0 else _DET_MU['body_atr'],
        'adx': float(r['ADX']) if not pd.isna(r['ADX']) else _DET_MU['adx'],
    }


def _prob_reversion_calibrada(df, tipo_posicion):
    feats = _features_reverso(df, tipo_posicion)
    if feats is None:
        return 50.0
    z = _DET_W['intercept']
    for f in _DET_FEATS:
        z += _DET_W[f] * (feats[f] - _DET_MU[f]) / _DET_SD[f]
    z = max(-30.0, min(30.0, z))
    p = 1.0 / (1.0 + math.exp(-z))
    return round(min(max(p * 100.0, 5.0), 95.0), 1)


def _prob_reversion_heuristica(df, tipo_posicion):
    """Detector ORIGINAL (heurístico, NO calibrado — corr ~0.02 con reverso real,
    de hecho ligeramente INVERTIDO). Conservado como fallback / referencia."""
    try:
        prob = 15.0
        rsi = df['RSI'].iloc[-1]
...
def prob_reversion(df, tipo_posicion):
    """Probabilidad (5-95%) de reverso inmediato contra la posición. Usa el
    detector CALIBRADO si config.DETECTOR_CALIBRADO (default), si no el
    heurístico viejo. Cualquier excepción → fallback heurístico."""
    if getattr(config, 'DETECTOR_CALIBRADO', False):
        try:
            return _prob_reversion_calibrada(df, tipo_posicion)
        except Exception:
            return _prob_reversion_heuristica(df, tipo_posicion)
    return _prob_reversion_heuristica(df, tipo_posicion)
...
def evaluar_entrada(df, rs_basket=None, rs_symbol=None, reentrada_armada=False):
    ...
    if modo == 'patrones':
        ...
        if not confirmacion:
            return None
        # V30 (2026-06-21, portado desde stable_v25_prototype para proyectar
        # su efecto bajo el EXIT_MODE='copilot' real de V28): filtro de
        # momentum SOLO para confirmaciones tipo 'reversion' — bloquear si
        # MACD_Hist está del lado contrario al trade. Default OFF; V28 en
        # vivo no cambia hasta que se decida desplegarlo.
    ...
    # V34 (2026-07-01): el detector calibrado usa solo features del estado
    # actual (no depende del avance de una posición), así que es evaluable
    # en el momento de la señal. Si la señal ya nace "agotada" (probabilidad
    # de reverso alta desde el arranque), se descarta. Reusa el umbral ya
    # establecido del salvavidas (PROB_REVERSION_MIN_CIERRE), no uno nuevo.
    prob_rev = prob_reversion(df, tendencia)
    if getattr(config, 'FILTRO_PROB_ENTRADA', False) \
            and prob_rev >= config.PROB_REVERSION_MIN_CIERRE:
        return None
...
def breakeven_info(t):
    """Precio de salida en el que el PnL neto (post-fees, ambos lados) es
    exactamente cero, y su equivalente en % del recorrido a TP (misma escala
    que calcular_progreso) — para avisar al trader cuánto falta/sobra antes
    de que 'tomar profit' sea realmente ganancia neta."""
    fee = config.TAKER_FEE
    if t['type'] == 'LONG':
        be = t['entry_price'] * (1 + fee) / (1 - fee)
    else:
        be = t['entry_price'] * (1 - fee) / (1 + fee)
    return be, calcular_progreso(t, be)


def evaluar_salida(t, precio, prob_reversion_actual=None):
    """
    ...
    Modo 'copilot' (V28, spec real — antes esta función no lo distinguía y
    caía en la lógica 'escalera' de arriba pese al comentario de config.py):
    solo STOP + TP duro al 100% (2R nominal); SIN decil/pullback automático —
    entre el SL y el TP la salida es decisión del trader, salvo el
    salvavidas opcional de AUTO_CIERRE_REVERSA (ver config.py).
    `prob_reversion_actual` es la heurística determinista ya existente
    (RSI/MACD/patrón opuesto/sobre-extensión), no un valor aleatorio — la
    pasa el llamador (ya tiene el df) porque esta función no lo recibe.
    """
    ...
    modo = getattr(config, 'EXIT_MODE', 'escalera')
    ...
    if modo == 'copilot':
        if prog >= 100:
            res['cerrar'] = 'OBJETIVO 100% ALCANZADO (2R)'
            return res
        decil = int(prog // config.LOCK_STEP_PCT) * config.LOCK_STEP_PCT
        if decil > t.get('locked_decile', 0) and decil >= config.LOCK_STEP_PCT:
            res['nuevo_decil'] = decil  # informativo — el decil asegurado
        # SALVAVIDAS ratchet+reverso (2026-06-24, spec del usuario): una vez
        # asegurado un decil D, si el avance RETROCEDE a tocar D (prog <= D) Y el
        # detector CALIBRADO de reverso confirma (>= umbral), se toma el profit
        # de inmediato. 'locked_decile' aquí es el decil asegurado en velas
        # ANTERIORES (el caller aplica res['nuevo_decil'] DESPUÉS de esta
        # función), así que la condición NO se auto-dispara al asegurar un decil
        # nuevo — solo cuando el avance vuelve a caer sobre el piso ya asegurado.
        # A diferencia de la vieja escalera de V25 (cierre incondicional por 8pts
        # de retroceso = trampa de costos), aquí el gate de reverso confirmado
        # evita cerrar en el ruido normal que luego sigue subiendo.
        locked = t.get('locked_decile', 0)
        if getattr(config, 'AUTO_CIERRE_REVERSA', False) and prob_reversion_actual is not None \
                and locked >= config.LOCK_STEP_PCT and prog <= locked \
                and prob_reversion_actual >= config.PROB_REVERSION_MIN_CIERRE:
            res['cerrar'] = (f"REVERSA CONFIRMADA: avance retrocedió a tocar el "
                             f"{locked}% asegurado con prob. reverso "
                             f"{prob_reversion_actual:.0f}% (≥{config.PROB_REVERSION_MIN_CIERRE}%) "
                             f"— profit tomado")
        return res

    # 'escalera' (legado V25) — deciles + cierre por pullback
    if prog >= 100:
