...
class BacktestV25:
    ...
    def _salidas_vela(self, t, row, ts, tendencia_ahora=None, sub_df=None):
        ...
        prob_actual = None
        if getattr(config, 'AUTO_CIERRE_REVERSA', False) and sub_df is not None:
    ...
    def _entrada_vela(self, sym, sub_df, ts, rs_basket=None, reentrada_armada=False):
        ...
        if cd is not None and ts < cd:
            return
        # Tarea #40: tope de posiciones concurrentes (limita exposición correlacionada)
        cap = getattr(config, 'MAX_CONCURRENT', None)
        if cap is not None:
            if sum(1 for x in self.trades if x['status'] == 'ABIERTA') >= cap:
                return
            ...
            for sym in symbols:
                ...
                for t in self.trades:
                    if t['symbol'] == sym and t['status'] == 'ABIERTA':
                        self._salidas_vela(t, row, ts, tendencia_ahora=tendencia_ahora, sub_df=sub)
                        if t['status'] == 'ABIERTA':
