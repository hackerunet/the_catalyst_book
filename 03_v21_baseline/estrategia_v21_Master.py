...
def detect_candle_pattern(df: pd.DataFrame, b_risk=0.002):
    """
...
def compute_signals_and_trades(df: pd.DataFrame, current_balance: float = 500.0, risk_percent: float = 0.004):
    """
    V3: Sistema de Dos Puertas Paralelas Bidireccional + Gestión de Riesgo Global (Max 2%).
    Puerta A (Alta Convicción): Breakout Donchian 24h + MA align + MACD. Riesgo: risk_percent (default 0.4%).
    Puerta B (Impulso de Vela): Marubozu/Engulfing + lado correcto MA99 + Volumen. Riesgo: 0.2% fijo. TP: 1R.
