...
from estrategia_v21_Master import compute_indicators, compute_signals_and_trades, INTERVAL, check_multi_candle_exhaustion
...
async def enviar_orden_binance(symbol, tipo_orden, qty, trade_id):
    ...
    # Ajuste de Precisión (Lotaje)
    qty_dec = ASSET_RULES.get(symbol, {}).get('qty_dec', 3)
    qty_fmt = round(qty, qty_dec)
    
    # Binance rechaza cantidades con .0 si la precisión exige entero (ej. XRP a veces)
    # pero el redondeo de Python a 'qty_dec' es suficiente para la API.
    ...
    try:
        ...
        if 'orderId' not in order_data:
            ...
            # Anulamos el trade en el Dashboard para no mantener la desincronización
...
async def cerrar_orden_binance(symbol, tipo_orden, qty_estimado):
    ...
    # Consultar cantidad exacta abierta para evitar decimales huérfanos
    try:
        ...
        # Si qty_real es 0, ya está cerrada (o fue cerrada por SL manual)
        if qty_real == 0.0:
    ...
    params = {
        ...
        'type': 'MARKET',
        'quantity': qty_fmt,
        'reduceOnly': 'true',  # Seguridad vital para no abrir posiciones accidentales
        'timestamp': int(time.time() * 1000)
    }
        ...
        if t['status'] == 'ABIERTA':
            ...
            if risk_amount > 0:
                ...
                if decil >= 10 and decil > t.get('last_reported_decil', 0):
                    ...
                    if is_ready:
                        print(f"ALERTA_PROGRESO|{t.get('id', '?')}|{symbol}|{decil}|{current_pnl:.4f}|{reversal_prob:.1f}", flush=True)
            ...
            if t['type'] == 'LONG':
                ...
                if candle['low'] <= t['sl']:
                    ...
                    if is_ready: 
                        print(f"ALERTA_CIERRE|{t['type']}|{symbol}|{t['exit_price']:.4f}|{t['pnl']:.4f}|{sim_state['balance']:.4f}", flush=True)
                        try:
                            asyncio.create_task(cerrar_orden_binance(symbol, t['type'], t['qty']))
                        except RuntimeError: pass
                        # --- FORENSE: POST-MORTEM DE CIERRE POR SL ---
                    ...
                    # PUERTA B: Activacion anticipada al llegar a 0.5R (conservadora)
                    puerta = t.get('puerta', 'A')
                    if puerta == 'B':
                        half_r = t['entry_price'] + (t['tp'] - t['entry_price']) * 0.5
                        if candle['high'] < half_r:
                            continue  # Aun no llega al umbral de Puerta B
                    t['status'] = 'RUNNER'
                    ...
                    if is_ready: 
                        print(f"ALERTA_PARCIAL|{t['type']}|{symbol}|{t['tp']:.4f}|+{partial_pnl:.4f}|RUNNER ACTIVO", flush=True)
                        try:
                            # Cerramos el 50% de la posición en Binance
                            asyncio.create_task(cerrar_orden_binance(symbol, t['type'], t['qty']))
                        except RuntimeError: pass
            ...
            if t['type'] == 'LONG':
                ...
                if candle['low'] <= t['sl']:
                    ...
                    if is_ready: 
                        print(f"ALERTA_CIERRE_RUNNER|{t['type']}|{symbol}|{t['exit_price']:.4f}|{runner_pnl:.4f}|Total PnL:{t['pnl']:.4f}", flush=True)
                        try:
                            asyncio.create_task(cerrar_orden_binance(symbol, t['type'], t['qty']))
                        except RuntimeError: pass
                        df_post = df_global.get(symbol, pd.DataFrame())
                                        ...
                                        for t in sim_state['trades']:
                                            if t['symbol'] == symbol_event and t['status'] == 'ABIERTA':
                                                if check_multi_candle_exhaustion(df_sym, t['type']):
                                                    ...
                                                    t['exit_reason'] = 'AGOTAMIENTO ESTRUCTURAL (V21)'
                                                    t['pnl'] = (t['exit_price'] - t['entry_price']) * t['qty'] if t['type'] == 'LONG' else (t['entry_price'] - t['exit_price']) * t['qty']
                                                    sim_state['balance'] += t['pnl']
                                                    try:
                                    ...
                                    if not trade_abierto_sym:
                                        ...
                                        if signal and trade_data:
                                            ...
                                            if recent_losses < 3 and current_balance >= 5.0:
                                                ...
                                                es_valido = True # Eliminada la validación síncrona, ahora será post-mortem
                                                ...
                                                if es_valido:
                                                    ...
                                                    qty = trade_data['qty']
                                                    puerta = trade_data.get('puerta', 'A')
                                                    patron_str = trade_data.get('pattern', f"{symbol_event} - V21")
                                                    
                                                    if is_ready:
                                                        print(f"ALERTA_TRADE|{signal}|{patron_str}|{c}|{current_balance}|{puerta}", flush=True)
                                                    ...
                                                    # --- FORENSE: REGISTRAR APERTURA CON CONTEXTO COMPLETO ---
    ...
    for t in sim_state['trades']:
        if t['symbol'] == symbol and t['status'] == 'ABIERTA':
            ...
            if current_price:
                ...
                if t['type'] == 'LONG':
                    t['pnl'] = (current_price - t['entry_price']) * t['qty']
                else:
                    t['pnl'] = (t['entry_price'] - current_price) * t['qty']
            else:
                t['exit_price'] = t['entry_price'] # Fallback
...
def update_dashboard(n, selected_symbol):
    ...
    if 'MA_7' in df_view.columns:
        fig.add_trace(go.Scatter(
            x=df_view['time'], y=df_view['MA_7'], mode='lines', name='MA 7',
            line=dict(color='#ff9900', width=1.5)
        ))
    if 'EMA_9' in df_view.columns:
        fig.add_trace(go.Scatter(
            x=df_view['time'], y=df_view['EMA_9'], mode='lines', name='EMA 9',
            line=dict(color='#ff33cc', width=1.5)
        ))
    if 'MA_99' in df_view.columns:
        fig.add_trace(go.Scatter(
            x=df_view['time'], y=df_view['MA_99'], mode='lines', name='MA 99 (Trend)',
            line=dict(color='#00bfff', width=2)
        ))
    if 'Donchian_High' in df_view.columns:
        fig.add_trace(go.Scatter(
            x=df_view['time'], y=df_view['Donchian_High'], mode='lines', name='Resistencia (Donchian High)',
            line=dict(color='#ffffff', width=1, dash='dash')
        ))
    if 'Donchian_Low' in df_view.columns:
        fig.add_trace(go.Scatter(
            x=df_view['time'], y=df_view['Donchian_Low'], mode='lines', name='Soporte (Donchian Low)',
            line=dict(color='#ffffff', width=1, dash='dash')
        ))
...
if __name__ == '__main__':
    ...
    # 5. Iniciar Dash
    print("Dash is running on http://127.0.0.1:8053/")
    app.run(port=8053, debug=False, use_reloader=False)
