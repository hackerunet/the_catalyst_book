...
class TelegramBot:
    ...
    def enviar(self, texto, botones_trade_id=None):
        payload = {'chat_id': self.chat_id, 'text': f"[V26] {texto}"}
        if botones_trade_id:
    ...
    def mensaje_alerta_roundtrip(self, t, prob_rev, prog_actual, tendencia_activa=None):
        ...
        lineas = [
            f"ℹ️ {icono} {t['symbol']} {t['type']} #{t['id']} — ROUND-TRIP · NO es señal de cerrar",
            f"• Llegó a un pico de {t.get('peak_progress', 0):.0f}% y ahora está en "
            f"{prog_actual:.0f}% (ya en negativo real, no solo un retroceso menor)",
            f"• Score de reverso (heurístico, NO calibrado): {prob_rev}%",
        ]
        if etiqueta_tend:
            lineas.append(f"• {etiqueta_tend}")
        lineas.append("Aviso informativo. V26 sigue gestionando sola (stop + flip). El 💰 es SOLO por si vos decidís cerrar a mano — NO es recomendación de cerrar.")
        return "\n".join(lineas)
