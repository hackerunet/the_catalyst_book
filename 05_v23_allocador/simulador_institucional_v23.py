...
# =============================================================================
# V23 — ASIGNADOR DE EXPOSICIÓN POR CALIDAD (quality-ranked allocator)
# =============================================================================
# Diferencia clave vs V22: cuando los 3 cupos de una dirección están llenos, V22
# RECHAZA la señal entrante (FIFO: el que llegó primero se queda con el cupo).
# Eso hace que una posición de BAJA convicción (p.ej. Puerta C=60) pueda BLOQUEAR
# una señal de ALTA convicción (Puerta A=95 / B=75) solo por haber llegado antes
# ("dead weight load-bearing" — ver el libro V22). V23 invierte la lógica: si la
# señal entrante tiene MAYOR convicción que la posición abierta MÁS DÉBIL en esa
# dirección, DESALOJA (cierra a mercado) la débil y admite la fuerte.
#
# IMPORTANTE: este allocator SOLO actúa en la ruta EN VIVO (donde múltiples símbolos
# tienen posiciones simultáneas reales). El backtest de este motor procesa los
# símbolos en SECUENCIA (no en tiempo global interleaved), así que el tope de
# exposición casi no liga ahí — por eso el allocator NO se aplica en el backtest
# (la ruta de backtest queda idéntica a V22 y su resumen debe reproducir el
# baseline V22). La validación de V23 es FORWARD en testnet, no por backtest.
# Convicción estática por puerta (A=95 > B=75 > C=60) => con '>' estricto solo
# A desaloja a B/C y B desaloja a C; nunca hay churn dentro de la misma puerta.

def posicion_mas_debil_misma_direccion(direction):
    """V23: posición abierta (ABIERTA o RUNNER) de la misma dirección con la MENOR convicción, o None."""
    abiertas = [tr for tr in sim_state['trades']
                if tr['type'] == direction and tr['status'] in ('ABIERTA', 'RUNNER')]
    if not abiertas:
        return None
    return min(abiertas, key=lambda tr: tr.get('conviccion', 0.0))


def desalojar_trade_vivo(t, nueva_conviccion, signal_entrante, simbolo_entrante):
    """V23 (en vivo): cierra a mercado la posición de menor convicción para liberar
    un cupo de exposición a favor de una señal entrante de MAYOR convicción.
    Replica exactamente la contabilidad de cierre de evaluate_open_trades (gross por
    dirección − fee_redondo, += a t['pnl'] para preservar parciales de RUNNER)."""
    precio = t['entry_price']
    df_sym = df_global.get(t['symbol'])
    if df_sym is not None and not df_sym.empty and 'close' in df_sym.columns:
        precio = float(df_sym['close'].iloc[-1])
    if t['type'] == 'LONG':
        gross = (precio - t['entry_price']) * t['qty']
    else:
        gross = (t['entry_price'] - precio) * t['qty']
    realizado = gross - fee_redondo(t['qty'], t['entry_price'], precio)
    t['pnl'] += realizado
    t['status'] = 'CERRADA'
    t['exit_time'] = pd.Timestamp.utcnow()
    t['exit_price'] = precio
    t['exit_reason'] = (f'DESALOJO POR CONVICCIÓN (allocator V23): entra '
                        f'{simbolo_entrante} {signal_entrante} conv {nueva_conviccion} '
                        f'> {t.get("conviccion", 0.0)}')
    sim_state['balance'] += realizado
    try:
        asyncio.create_task(cerrar_orden_binance(t['symbol'], t['type'], t['qty']))
    except RuntimeError:
        pass
    print(f"ALLOCATOR_DESALOJO|{t['symbol']}|{t['type']}|conv_debil {t.get('conviccion', 0.0)}|"
          f"pnl {realizado:.4f}|balance {sim_state['balance']:.4f}|"
          f"entra {simbolo_entrante} {signal_entrante} conv {nueva_conviccion}", flush=True)
                            ...
                            if not df_sym.empty:
                                ...
                                if is_candle_closed:
                                    ...
                                    if not trade_abierto_sym:
                                        ...
                                        if signal and trade_data:
                                            # --- V23: ASIGNADOR POR CALIDAD (reemplaza el FIFO de V22) ---
                                            # Si los cupos de esta dirección están llenos, NO rechazamos sin
                                            # más: comparamos la convicción de la entrante contra la posición
                                            # abierta MÁS DÉBIL. Si la entrante es estrictamente mayor, la
                                            # desalojamos (cierre a mercado) y admitimos la fuerte. Si no,
                                            # se bloquea como antes.
                                            puede_admitir = True
                                            if contar_posiciones_por_direccion(signal) >= MAX_SAME_DIRECTION_POSITIONS:
                                                debil = posicion_mas_debil_misma_direccion(signal)
                                                nueva_conv = trade_data.get('conviccion', 0.0)
                                                if debil is not None and nueva_conv > debil.get('conviccion', 0.0):
                                                    desalojar_trade_vivo(debil, nueva_conv, signal, symbol_event)
                                                else:
                                                    puede_admitir = False
                                                    print(f"FILTRO_CORRELACION|{symbol_event}|{signal}|Bloqueado: {MAX_SAME_DIRECTION_POSITIONS} posiciones {signal} abiertas y la entrante (conv {nueva_conv}) no supera a la más débil (conv {debil.get('conviccion', 0.0) if debil else 'N/A'})", flush=True)
...
# =========================================================================
app = dash.Dash(__name__, title="Quant Terminal V23 - Régimen Adaptativo")
...
app.layout = html.Div([
    html.H2("Terminal Cuantitativa Institucional V23 - Capa de Régimen Adaptativo", style={'textAlign': 'center', 'color': '#00ffcc', 'fontFamily': 'monospace'}),
...
def update_dashboard(n, selected_symbol):
    if not is_ready:
        empty_fig = go.Figure().update_layout(title="Iniciando escaneo multiplexado V23...", template='plotly_dark', paper_bgcolor='#0a0a0a', plot_bgcolor='#121212')
...
if __name__ == '__main__':
    print("STATUS: Iniciando Motor Multiplexado V23 [CAPA DE RÉGIMEN ADAPTATIVO]...")
    ...
    print("Dash V23 is running on http://127.0.0.1:8055/")
