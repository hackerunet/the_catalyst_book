...
# Tanda V60-V67 econofísica (2026-07-09, pre-registro en el libro): gate de régimen
# sobre las ENTRADAS (bloquea direcciones de entrada por vela). None = inerte
# (default — el bot vivo y todo lo previo no cambian). Valores:
# 'entropia'|'mfdfa'|'lppls'|'omori'|'rmt'|'dispersion'|'funding'.
# Solo walkforward.py --gate lo activa (harness). Ver gates_econofisica.py.
GATE_ECONO = None

# V63 (tanda econofísica): señal de ENTRADA por transfer entropy líder→alt
# (BTC como líder; medición bidireccional per pre-registro). False = inerte.
# Solo walkforward.py --te-leadlag lo activa. Ver gates_econofisica.senales_te.
TE_LEADLAG = False

# --- V68 (2026-07-10): knobs de la AUDITORÍA DE ROBUSTEZ DE RECHAZOS ---
# Env-vars con default = comportamiento canónico EXACTO (no-regresión garantizada
# si no se setean). config.py es el único lector de entorno (regla del proyecto).
# Pre-registro en el libro sección V68. Solo el piloto los usa.
DONCHIAN_N = int(os.getenv('V68_DONCHIAN_N', '20'))
DONCHIAN_PURO = os.getenv('V68_DONCHIAN_PURO', '0') == '1'   # ruptura sin exigir tendencia
SR_PERF_VELAS = int(os.getenv('V68_SR_PERF_VELAS', '2'))     # velas que pudieron perforar la BB
SR_REQ_BB = os.getenv('V68_SR_REQ_BB', '1') == '1'           # exigir la perforación Bollinger
SR_REQ_CIERRE_BANDA = os.getenv('V68_SR_REQ_CIERRE_BANDA', '1') == '1'  # "no cierra fuera de banda"
SR_SWING_N = int(os.getenv('V68_SR_SWING_N', '15'))          # 15 canónico | 10 variante
SR_REQ_RSI_MOM = os.getenv('V68_SR_REQ_RSI_MOM', '1') == '1'
MS_REQ_PULLBACK = os.getenv('V68_MS_REQ_PULLBACK', '1') == '1'
MS_REQ_PATRON = os.getenv('V68_MS_REQ_PATRON', '1') == '1'
MS_REQ_RSI_MOM = os.getenv('V68_MS_REQ_RSI_MOM', '1') == '1'
MS_REQ_VOL = os.getenv('V68_MS_REQ_VOL', '1') == '1'
MS_REQ_SIGNO = os.getenv('V68_MS_REQ_SIGNO', '1') == '1'     # exigir MACD_Hist del signo correcto
...
# CLÍMAX (2026-07-08, ingeniería inversa de los picos de monstruos): la firma
# distintiva del pico de un monstruo NO es el fade de momentum (ADX cae solo 8%
# en el pico vs 53% en velas normales) sino la SOBRE-EXTENSIÓN / clímax: en el
# candle-pico, RSI extremo (>70 dir en 66-73% de los picos vs 5-9% base),
# cierre FUERA de Bollinger (51-52% vs 6-7%) y clímax de volumen (77-80% vs 17%).
# Salida = cerrar al CIERRE de vela, en trade con ganancia, si coinciden las 3
# (AND, para base baja). Familia nueva (agotamiento por euforia), opuesta a todo
# lo rechazado. Umbrales redondos ya usados en el proyecto (RSI 70, vol 1.5×), NO
# escaneados. Solo walkforward.py lo activa; V26/V36 vivos NO cambian.
CLIMAX_EXIT_TENDENCIA = False
CLIMAX_RSI = 70.0          # RSI extremo en la dirección del trade
CLIMAX_VOL_RATIO = 1.5     # volumen >= 1.5x su media (clímax)

# ---------------------------------------------------------------------------
# V71 (2026-07-17) — salida por APAGADO DE CLÍMAX. Máquina de DOS estados:
# ARMA cuando el RSI cierra >70 (LONG) / <30 (SHORT); DISPARA cuando el RSI
# cierra de vuelta del otro lado. Distinto de V58 (que salía MIENTRAS RSI>70,
# o sea EN el techo, con BB+volumen en AND) y de Test 2 (divergencia).
# Motivación: en el monstruo real de ETH (3105a500) el RSI (74.9) fue el ÚNICO
# indicador que marcó el techo en tiempo real — ADX y MACD picaron DESPUÉS
# (el ADX 16 h tarde) y el flip nunca disparó.
# Se evalúa ANTES del flip (el flip queda como red de seguridad), al CIERRE de
# vela. El stop conserva prioridad (intravela pesimista).
# Dos variantes — NUNCA las dos a la vez:
CLIMAX_FADE_EXIT_TENDENCIA = False   # V71-A: activo desde el inicio del trade
CLIMAX_FADE_FUNNEL = False           # V71-B: activo SOLO tras round-trip + pico nuevo
# V71-B (idea del usuario): el flip manda en la PRIMERA pierna (ahí vive el edge:
# dejar correr). Si el trade ya demostró que hace round-trip (picó >=
# FUNNEL_RT_MIN_PICO y volvió a NEGATIVO) y DESPUÉS creció por encima de ese
# pico, el sistema se vuelve desconfiado: en la segunda pierna manda el RSI y el
# flip pasa a segundo. Ningún rechazo previo hacía esto — todos cortaban desde
# el minuto cero. Umbral 50 = ALERTA_ROUNDTRIP_MIN_PICO (ya usado en V26/V36),
# no un número nuevo.
FUNNEL_RT_MIN_PICO = 50.0
# ---------------------------------------------------------------------------
