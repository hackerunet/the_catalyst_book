...
class BacktestV25:
    ...
    def _salidas_vela(self, t, row, ts, tendencia_ahora=None):
        ...
        if getattr(config, 'EXIT_MODE', 'escalera') == 'tendencia':
            ...
            # CLÍMAX (2026-07-08): cerrar al CIERRE de vela, con el trade EN
            # GANANCIA, si coinciden las 3 señales del pico de monstruo (RSI
            # extremo en la dirección + cierre FUERA de Bollinger + clímax de
            # volumen). Causal: usa indicadores de la vela YA cerrada (row), sin
            # look-ahead. AND de las 3 = base baja (la individual se prende en
            # 6-24% de las velas normales; juntas casi nunca salvo en un blow-off).
            if getattr(config, 'CLIMAX_EXIT_TENDENCIA', False):
                sign = 1.0 if t['type'] == 'LONG' else -1.0
                en_ganancia = (row['close'] - t['entry_price']) * sign > 0
                rsi_dir = row['RSI'] if t['type'] == 'LONG' else 100.0 - row['RSI']
                fuera_bb = row['close'] > row['BB_UPPER'] if t['type'] == 'LONG' \
                    else row['close'] < row['BB_LOWER']
                vol_climax = row['Volume_MA'] > 0 and \
                    row['volume'] >= row['Volume_MA'] * getattr(config, 'CLIMAX_VOL_RATIO', 1.5)
                if en_ganancia and rsi_dir > getattr(config, 'CLIMAX_RSI', 70.0) \
                        and fuera_bb and vol_climax:
                    self._cerrar(t, row['close'], ts, 'CLIMAX (RSI+BB+vol)')
                    return
            ...
            # V71: APAGADO DE CLÍMAX — se evalúa ANTES del flip (el flip queda de
            # red de seguridad, per el diseño del "funnel"). Estado en el trade,
            # causal (RSI de la vela YA cerrada).
            motivo_fade = estrategia.salida_por_apagado_climax(t, row, tendencia_ahora)
            if motivo_fade:
                self._cerrar(t, row['close'], ts, motivo_fade)
                return
    ...
    def _entrada_vela(self, sym, sub_df, ts, rs_basket=None, reentrada_armada=False):
        ...
        if cd is not None and ts < cd:
            return
        # V63 (econofísica): si hay señales de transfer entropy inyectadas, ELLAS son
        # la entrada (reemplazan a evaluar_entrada); sizing/guardas por _armar_senial
        # — el MISMO code-path de la familia (no una réplica). Ausente = inerte.
        _ste = getattr(self, 'senales_te', None)
        if _ste is not None:
            _dir = _ste.get(sym, {}).get(ts)
            if _dir is None:
                return
            señal = estrategia._armar_senial(sub_df, _dir, 'TE lead-lag (líder BTC)', [])
        else:
            señal = estrategia.evaluar_entrada(sub_df, rs_basket=rs_basket, rs_symbol=sym,
                                               reentrada_armada=reentrada_armada)
        if not señal:
            return
        # Gate econofísica V60-V67 (2026-07-09): dict {sym: {ts: 'ALL'|'LONG'|'SHORT'}}
        # inyectado por walkforward (--gate). Ausente/None = inerte (no-regresión).
        # El null MC aplica EL MISMO dict (un solo code-path).
        _g = getattr(self, 'gates_econo', None)
        if _g is not None:
            _b = _g.get(sym, {}).get(ts)
            if _b is not None and (_b == 'ALL' or _b == señal['type']):
                return
