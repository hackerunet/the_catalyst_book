...
from estrategia_v24a_Master import compute_indicators, compute_signals_and_trades, INTERVAL, check_multi_candle_exhaustion, classify_regime
...
# riesgo por disparador (A=1%, B=1%) ahora vive en estrategia_v24a_Master,
...
def evaluate_open_trades(candle, symbol, current_atr, ma99=None, macd_hist=None):
    ...
    for t in sim_state['trades']:
        ...
        if t['status'] == 'ABIERTA':
            ...
            # --- 2) TRADE SIGUE ABIERTO: time stop, progreso ---
            # TIME STOP (V24A): un trade que en `time_stop_horas` nunca alcanzó
            # +0.5R no tiene la tesis funcionando — paga funding y bloquea un
            # cupo de exposición. Cierre a mercado al cierre de vela. Usa el
            # riesgo INICIAL (el sl puede haberse movido a breakeven).
            ts_horas = t.get('time_stop_horas')
            if ts_horas and t.get('entry_time') is not None:
                riesgo_ini = t.get('risk_inicial', risk_amount)
                if riesgo_ini > 0:
                    try:
                        horas_abierto = (_naive_utc(pd.Timestamp(candle['time'])) - _naive_utc(pd.Timestamp(t['entry_time']))) / pd.Timedelta(hours=1)
                    except Exception:
                        horas_abierto = 0
                    if horas_abierto >= ts_horas:
                        if t['type'] == 'LONG':
                            mfe_ok = t.get('highest_price', t['entry_price']) >= t['entry_price'] + riesgo_ini * 0.5
                        else:
                            mfe_ok = t.get('lowest_price', t['entry_price']) <= t['entry_price'] - riesgo_ini * 0.5
                        if not mfe_ok:
                            t['status'] = 'CERRADA'
                            t['exit_time'] = candle['time']
                            t['exit_price'] = candle['close']
                            t['exit_reason'] = 'TIME STOP (48h sin progreso)'
                            t['pnl'] = cerrar_tramo(t, t['qty'], t['exit_price'], candle['time'])
                            sim_state['balance'] += t['pnl']
                            _notificar_cierre(t, symbol, t['pnl'])
                            continue
            ...
            # --- 3) BREAKEVEN A +1R — armado AL FINAL de la vela, efectivo
            # desde la siguiente. V24A: aplica a TODOS los trades (estructura
            # de salida única del Sistema de Momentum; en V22/V24 solo a 'A').
            if not t.get('breakeven_armed') and risk_amount > 0:
...
def _abrir_trade_backtest(sub_df, symbol, current_time):
    ...
    if recent_losses < 3 and sim_state['balance'] >= 5.0:
        ...
        sim_state['trades'].append({
            ...
            'risk_inicial': abs(trade_data['entry_price'] - trade_data['stop_loss']),
            'time_stop_horas': trade_data.get('time_stop_horas'),
            'qty': trade_data['qty'],
...
# =========================================================================
app = dash.Dash(__name__, title="Quant Terminal V24A - Sistema Único de Momentum")
...
app.layout = html.Div([
    html.H2("Terminal Cuantitativa Institucional V24A - Sistema Único de Momentum", style={'textAlign': 'center', 'color': '#00ffcc', 'fontFamily': 'monospace'}),
...
def update_dashboard(n, selected_symbol):
    if not is_ready:
        empty_fig = go.Figure().update_layout(title="Iniciando escaneo multiplexado V24A...", template='plotly_dark', paper_bgcolor='#0a0a0a', plot_bgcolor='#121212')
...
if __name__ == '__main__':
    print("STATUS: Iniciando Motor Multiplexado V24A [SISTEMA ÚNICO DE MOMENTUM sobre motor honesto V24]...")
    ...
    print("Dash V24A is running on http://127.0.0.1:8057/")
    app.run(port=8057, debug=False, use_reloader=False)
