...
# --- V24A: nombres internos actualizados (referencias de log + servicio objetivo) ---
LOG_FILE = 'v24a_live.log'
SIMULADOR_TARGET = "simulador_institucional_v24a.py"
...
# V24A — Sistema Único de Momentum: A y B son DISPARADORES del mismo sistema
# (misma estructura de salida), no sub-estrategias separadas. No existe Puerta C.
PUERTA_LABELS = {
    'A': 'Momentum — disparador Breakout (Donchian + tendencia confirmada)',
    'B': 'Momentum — disparador Impulso (Marubozu/Engulfing en tendencia)',
}

PUERTA_ICONOS = {'A': '🟢', 'B': '🔵'}
PUERTA_RIESGO = {'A': '1%', 'B': '1%'}
...
def send_telegram(mensaje, reply_markup=None):
    ...
    payload = {"chat_id": TELEGRAM_TARGET, "text": f"[V24A_MOMENTUM] {mensaje}"}
    if reply_markup:
...
def telegram_polling():
    ...
    print("MESA DE DINERO V24A: Iniciando polling interactivo de Telegram...")
    while True:
        try:
                ...
                for update in data['result']:
                    ...
                    if 'callback_query' in update:
                        ...
                        if cb_data.startswith("close_trade|"):
                            ...
                            try:
                                # V24A corre su dashboard/API en el puerto 8057 (8053=V21, 8054=V22, 8055=V23, 8056=V24)
                                r = requests.post("http://127.0.0.1:8057/api/close_trade", json={'symbol': symbol}, timeout=5)
...
def run_mesa():
    log_diario("MESA DE DINERO V24A: Inicializando servicio autónomo (Sistema Único de Momentum).")
    send_telegram(
        "🚀 V24A OPERATIVA — Sistema Único de Momentum (rediseño desde conclusiones)\n"
        "• UN solo sistema: momentum alineado con tendencia (lo único con edge robusto en V22)\n"
        "• Filtro: EMA9/MA99 alineadas + pendiente de MA99 en la dirección del trade\n"
        "• Disparadores: Breakout Donchian (2 velas + Body≥ATR + vol≥1.5x + RSI 80/20) e Impulso (Marubozu/Engulfing + vol + RSI 70/30)\n"
        "• Salida única: stop 2.5x ATR (1.5%-4%), 50% a 2R + runner + breakeven +1R, TIME STOP 48h\n"
        "• ELIMINADO: Puerta C / reversión a la media (sin edge OOS) y el gating por régimen\n"
        "• Motor honesto V24: reloj global + intravela pesimista + comisión/slippage/funding\n"
    ...
    while True:
        print(f"MESA DE DINERO V24A: Iniciando proceso de {SIMULADOR_TARGET}...")
        ...
        while True:
            ...
            #     puerta y por régimen (la pieza clave para "determinar la mejor estrategia").
                ...
                try:
                    ...
                    bloques = [
                        f"{icono_resultado} RESULTADOS DEL BACKTEST HISTÓRICO — V24A (Sistema Único de Momentum)\n"
            ...
            #     ahora informado por la Puerta y el Régimen que originaron la operación.
                ...
                if len(partes) >= 6:
                    ...
                    label = PUERTA_LABELS.get(puerta, 'Sub-estrategia desconocida')
                    regimen_label = REGIMEN_LABELS.get(regimen, regimen)
                    # V24A: estructura de salida única — ambos disparadores van a 2R+runner
                    objetivo_str = "2R parcial + runner con trailing (salida única V24A)"
                    msg = (
