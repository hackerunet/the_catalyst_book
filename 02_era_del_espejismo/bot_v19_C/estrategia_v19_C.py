...
# === CONFIGURACIÓN GLOBAL DEL MOTOR V15 (ESTABILIDAD ESTRUCTURAL 1H/4H) ===
INTERVAL = '1h'
LIMIT = 1000
...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 2. EMAs Macro (Simulando 4H en 1H)
    # EMA 21 en 4H = EMA 84 en 1H
    # EMA 50 en 4H = EMA 200 en 1H
    df['EMA_84'] = df['close'].ewm(span=84, adjust=False).mean()
    df['EMA_200'] = df['close'].ewm(span=200, adjust=False).mean()
    
    # 3. EMA Micro (Tendencia local 1H)
    ...
    # 5. SMA de Volumen (20 periodos) para validación institucional
    ...
    # 7. Swing High / Low de 15 periodos para V19_C
    df['Swing_Low_15'] = df['low'].rolling(window=15).min()
    df['Swing_High_15'] = df['high'].rolling(window=15).max()
...
def detect_v19_C_setup(df: pd.DataFrame, direction: str):
    if len(df) < 25:
        return False, 0.0, ""
    ...
    if direction == 'LONG':
        # Filtro Macro (4H)
        if curr['EMA_84'] < curr['EMA_200']: return False, 0.0, ""
        
        # Filtro V18_A: Bollinger Extremo
        if curr['low'] > curr['BB_LOWER'] and prev['low'] > prev['BB_LOWER']: return False, 0.0, ""
        
        # Filtro V19_C: Swing Rejection (Caza de Liquidez)
        prev_swing_low = prev['Swing_Low_15']
        if curr['low'] > prev_swing_low or curr['close'] <= curr['BB_LOWER']: return False, 0.0, ""
        
        # Filtro V17_C: RSI Momentum (Mantenemos como capa base)
        if curr['RSI'] < prev['RSI']: return False, 0.0, ""
        ...
        if pullback and (has_hammer or has_engulfing or has_mstar):
            # Filtro V16: Confirmación por Volumen
            if curr['volume'] < curr['Vol_SMA_20']:
                return False, 0.0, ""
        ...
        # Filtro V18_A: Bollinger Extremo
        if curr['high'] < curr['BB_UPPER'] and prev['high'] < prev['BB_UPPER']: return False, 0.0, ""
        
        # Filtro V19_C: Swing Rejection (Caza de Liquidez)
        prev_swing_high = prev['Swing_High_15']
        if curr['high'] < prev_swing_high or curr['close'] >= curr['BB_UPPER']: return False, 0.0, ""
        
        # Filtro V17_C: RSI Momentum
        if curr['RSI'] > prev['RSI']: return False, 0.0, ""
...
def compute_signals_and_trades(df: pd.DataFrame, current_balance: float, risk_percent: float):
    ...
    is_long, conviccion_long, patron_long = detect_v19_C_setup(df, 'LONG')
    if is_long and rsi < 65: # Relajamos el RSI para capturar pullbacks en tendencias fuertes
        ...
        return 'LONG', {
            ...
            'metrics': {'strategy': f'v16_long_{patron_long.replace(" ", "_").lower()}', 'sl_dist': sl_val}
        }
        
    is_short, conviccion_short, patron_short = detect_v19_C_setup(df, 'SHORT')
    if is_short and rsi > 35:
        ...
        return 'SHORT', {
            ...
            'metrics': {'strategy': f'v16_short_{patron_short.replace(" ", "_").lower()}', 'sl_dist': sl_val}
        }
