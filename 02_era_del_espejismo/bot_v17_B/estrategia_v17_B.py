...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 6. RVWAP V17_B (Rolling VWAP de 24 periodos)
    typical_price = (df['high'] + df['low'] + df['close']) / 3
    df['VWAP_24'] = (typical_price * df['volume']).rolling(window=24).sum() / (df['volume'].rolling(window=24).sum() + 1e-10)
...
def detect_v17_B_setup(df: pd.DataFrame, direction: str):
    if len(df) < 25:
        return False, 0.0, ""
    ...
    if direction == 'LONG':
        ...
        # Filtro V17_B: VWAP Institucional
        if curr['close'] < curr['VWAP_24']: return False, 0.0, ""
...
def compute_signals_and_trades(df: pd.DataFrame, current_balance: float, risk_percent: float):
    ...
    is_long, conviccion_long, patron_long = detect_v17_B_setup(df, 'LONG')
    ...
    is_short, conviccion_short, patron_short = detect_v17_B_setup(df, 'SHORT')
