...
from estrategia_v13 import compute_indicators, compute_signals_and_trades, INTERVAL
...
# =========================================================================
PORTFOLIO = {
    'BTCUSDT': {'risk': 0.002}, # 0.2% por trade
    'ETHUSDT': {'risk': 0.002},
    'SOLUSDT': {'risk': 0.002},
    'BNBUSDT': {'risk': 0.002},
    'XRPUSDT': {'risk': 0.002}
    ...
    for t in sim_state['trades']:
        if t['status'] == 'ABIERTA' and t['symbol'] == symbol:
            ...
            if t['type'] == 'LONG':
                ...
                # Salidas SET-AND-FORGET
                if candle['low'] <= t['sl']:
                    exit_price = t['sl']
                    exit_reason = 'STOP LOSS (Riesgo Asumido)'
                elif candle['high'] >= t['tp']:
                    exit_price = t['tp']
                    exit_reason = 'TAKE PROFIT (Objetivo Alcanzado)'
            ...
            if exit_price is not None:
                ...
                # Calculamos PnL: Si es LONG (Exit - Entry), si es SHORT (Entry - Exit)
                pnl_raw = (exit_price - t['entry_price']) if t['type'] == 'LONG' else (t['entry_price'] - exit_price)
                t['pnl'] = pnl_raw * t['qty']
