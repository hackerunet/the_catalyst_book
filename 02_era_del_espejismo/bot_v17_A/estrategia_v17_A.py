...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    plus_di = 100 * (pd.Series(plus_dm).ewm(alpha=1/14, adjust=False).mean() / atr_14)
    minus_di = 100 * (pd.Series(minus_dm).ewm(alpha=1/14, adjust=False).mean() / atr_14)
...
def detect_v17_A_setup(df: pd.DataFrame, direction: str):
    if len(df) < 25:
        return False, 0.0, ""
    ...
    if direction == 'LONG':
        ...
        # Filtro V17_A: ADX Fuerte
        if curr['ADX'] < 25: return False, 0.0, ""
...
def compute_signals_and_trades(df: pd.DataFrame, current_balance: float, risk_percent: float):
    ...
    is_long, conviccion_long, patron_long = detect_v17_A_setup(df, 'LONG')
    ...
    is_short, conviccion_short, patron_short = detect_v17_A_setup(df, 'SHORT')
