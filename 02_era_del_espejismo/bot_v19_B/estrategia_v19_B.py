...
def detect_v19_B_setup(df: pd.DataFrame, direction: str):
    if len(df) < 25:
        return False, 0.0, ""
    ...
    if direction == 'LONG':
        ...
        # Filtro V19_B: Squeeze Release (Volatilidad en expansión)
        if curr['BB_STD'] <= prev['BB_STD']: return False, 0.0, ""
...
def compute_signals_and_trades(df: pd.DataFrame, current_balance: float, risk_percent: float):
    ...
    is_long, conviccion_long, patron_long = detect_v19_B_setup(df, 'LONG')
    ...
    is_short, conviccion_short, patron_short = detect_v19_B_setup(df, 'SHORT')
