...
def detect_v16_setup(df: pd.DataFrame, direction: str):
    if len(df) < 5:
        return False, 0.0, ""
...
def compute_signals_and_trades(df: pd.DataFrame, current_balance: float, risk_percent: float):
    ...
    is_long, conviccion_long, patron_long = detect_v16_setup(df, 'LONG')
    ...
    is_short, conviccion_short, patron_short = detect_v16_setup(df, 'SHORT')
