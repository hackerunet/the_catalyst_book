...
def detect_v18_C_setup(df: pd.DataFrame, direction: str):
    if len(df) < 5:
        return False, 0.0, ""
    ...
    if direction == 'LONG':
        ...
        # Filtro V18_C: MACD Momentum (Histograma creciente positivo)
        if curr['MACD_Hist'] <= 0 or curr['MACD_Hist'] < prev['MACD_Hist']: return False, 0.0, ""
        
        # Filtro V17_C: RSI Momentum (Mantenemos como base)
        ...
        # Filtro V18_C: MACD Momentum (Histograma decreciente negativo)
        if curr['MACD_Hist'] >= 0 or curr['MACD_Hist'] > prev['MACD_Hist']: return False, 0.0, ""
...
def compute_signals_and_trades(df: pd.DataFrame, current_balance: float, risk_percent: float):
    ...
    is_long, conviccion_long, patron_long = detect_v18_C_setup(df, 'LONG')
    ...
    is_short, conviccion_short, patron_short = detect_v18_C_setup(df, 'SHORT')
